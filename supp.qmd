...
pacman::p_load(terra, sf, rnaturalearth, mapview, raster, NicheMapR, lutz, tidyverse, magick, ggimage, grid, stars, microclima, cowplot, ggpubr, patchwork, cowplot, geodata)
...
# Behavioural traits (needed for Activity Models)
    ...
    nocturn <- 0                 # Nocturnal activity allowed (1) or not (0)?
     ...
     maxdepth <- 10              # Maximum burrowing depth
...
extract_times_activity <- function(x){
  ...
  # Check activity
  per_active_day <- environ_lizard  %>% filter(dates > date_min & dates < date_max) %>% filter(TIME > 6 & TIME < 19) %>%
    group_by(DAY) %>%
    summarise(TSM = mean(CT_max - TC))
...
for(i in 1:length(list_process)){
  ...
  p1_list[[i]] <- 
    ...
    scale_colour_manual(name = "",
                        values = c("Air temperature (2 m)" = "#000000", 
                                   "Body temperature" = "#DAA520",
                                   "Rainfall" = "#87CEEB"),
                        labels = c("Air Temperature", "Body Temperature", "Rain")) +
    annotate("text", x = as.POSIXct(date_max)-1e6, y = 45, # change if date changes
             label = paste0("Thermal Safety Margin: ",
                            round(median(per_active_day$TSM), digits = 1)), hjust = 0) +
    theme_classic() + 
    theme(legend.position = "none", 
          axis.title = element_blank()) + theme(axis.text = element_text(size = 24),
  ...
  p2_list[[i]] <-
    ggplot() +
    geom_line(data = rain_lizard2, aes(y = rainfall, x = dates), colour = "#87CEEB") +
    labs(y = "Rainfall (mm)", x = "Date") +
    theme_classic() + theme(axis.title = element_blank(),
    panel.background = element_rect(fill = "transparent", colour = NA),
    plot.background  = element_rect(fill = "transparent", colour = NA), legend.position = "none", axis.text = element_text(size = 24))
}
...
# p1 <- p1_list[[1]] 
# p2 <- p1_list[[2]] + theme(legend.position = "none")
...
#   inset_element(p1, left = -0.52,  right = 0.08, bottom = 0.52, top = 1.05)+
#   inset_element(p2, left = 0.10,  right = 0.7, bottom = 0.40, top = 0.93)+
#   plot_annotation(tag_levels = 'A') 
# plot_final

# ## Save wrap_elements(
# ggsave(extreme_map, 
#        filename = "./outdir/extreme_heat_sleepy_lizard_microclimate.png", 
#        width = 10, height = 7, bg = "transparent")

ggsave(p1_list[[1]], 
       filename = "./outdir/extreme_heat_sleepy_lizard_microclimate_inset.png", 
       width = 10, height = 7, bg = "transparent")

ggsave(p1_list[[2]], 
       filename = "./outdir/extreme_heat_sleepy_lizard_microclimate_inset2.png", 
       width = 10, height = 7, bg = "transparent")

```


# Comparing to Worldclime Bio 1
...
# Load WorldClim Bio1  (mean annual temperature) and Bio5 (Max Annual Temp - more realistic) data
    bio <- worldclim_country(country = "AUS", var = "bio", res = 0.5, path = "./indir/worldclim/")
    bio5 <- bio[[5]]  # Bio5 - Max Temperature of Warmest Month
    bio1 <- bio[[1]]  # Bio1 - Mean Annual Temperature

# Now, we need to extract Bio5 values for our points of interest in the sleepy lizard range

  # make sure CRS's match between WC and points
         coords <- data.frame(x = points$x, y = points$y)
         coords <- sf::st_as_sf(coords, coords = c("x", "y"), crs = 4326)
         coords <- sf::st_transform(coords, crs = terra::crs(bio5)) 
  
  # Extact values
    bio5_values <- terra::extract(bio5, coords, xy = TRUE)
    bio1_values <- terra::extract(bio1, coords, xy = TRUE)

# Now calculate TSM for Bio5
bio5_values <- bio5_values %>%
  mutate(CT_max = 42) %>%
  mutate(TSM_bio5 = CT_max - wc2.1_30s_bio_5)

bio1_values <- bio1_values %>%
  mutate(CT_max = 42) %>%
  mutate(TSM_bio1 = CT_max - wc2.1_30s_bio_1)
