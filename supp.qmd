...
pacman::p_load(terra, sf, rnaturalearth, mapview, raster, NicheMapR, lutz, tidyverse, magick, ggimage, grid, stars, microclima, cowplot, ggpubr)
...
for(i in 1:length(list_process)){
  ...
  p1_list[[i]] <- 
    ...
    geom_line(data = rain_lizard2, aes(y = rainfall*2, x = dates, colour = "Rainfall")) +
    ...
    annotate("text", x = as.POSIXct(date_max)-1e6, y = 10, # change if date changes
             label = paste0("Median Daily Activity (%): ",
                            round(median(per_active_day$per_active), digits = 2)), hjust = 0) +
    theme_classic() + 
...
ggarrange(p2_list[[1]], p2_list[[2]])

top <- extreme_map
bottom <- ggarrange(p1_list[[1]], p1_list[[2]], ncol = 2, common.legend = TRUE, legend = "bottom")
plot_grid(top, bottom, nrow = 2, rel_heights = c(1.5,1))




# For sleepy lizard inset picture, wherever that goes     
